# Extra logistics consistency rules used for the extra AIJ experiments
# We use the same continuous consistency rules as in logistics_consistency.py
# To the eventual consistency rules, we add the following:
# - Each location/airport must contain at least one package
...
class ConsistencyEvaluatorLogisticsExtra(ConsistencyEvaluator):
    ...
    def check_eventual_consistency(self, curr_state):
        ...
        # Each location/airport must contain at least one package
        formula_4 = FA(x, (_type(x, location) | _type(x, airport)) >> TE(y, _type(y, package) & at(y, x)))
