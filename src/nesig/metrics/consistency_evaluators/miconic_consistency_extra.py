# Extra miconic consistency rules used for the extra AIJ experiments
# We use the same continuous consistency rules as in miconic_consistency.py
# To the eventual consistency rules, we add the following:
# - There must be at least four floors and four passengers

from ..consistency import ConsistencyEvaluator
from pddl_prover import *

class ConsistencyEvaluatorMiconicExtra(ConsistencyEvaluator):
    ...
    def check_eventual_consistency(self, curr_state):
        ...
        # There must be at least four floors
        formula_3 = TE(x, _type(x, floor)) >= 4

        # There must be at least four passengers
        formula_4 = TE(x, _type(x, passenger)) >= 4
