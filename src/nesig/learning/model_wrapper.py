...
class NLMWrapper(ModelWrapperActorCritic):
    ...
    def _stack_state_encodings(self, list_state_encodings:List[List[Optional[torch.Tensor]]]):
        # max_num_objs = max([p._initial_state.num_objects for p in problems])

        # Calculate the maximum number of objects
        # Obtain the arity (starting from unary) for which tensors are None
        inds_no_None = [i+1 for i,t in enumerate(list_state_encodings[0][1:]) if t is not None]

        if len(inds_no_None) == 0: # There are only nullary tensors, so we do not need to pad
            list_padded_state_encodings = list_state_encodings
        else:
            ind_no_None = inds_no_None[0]
            max_num_objs = max([tensor_list[ind_no_None].shape[0] for tensor_list in list_state_encodings])
    ...
    def obtain_internal_state_encodings(self, problems:List[PDDLProblem]):
        ...
        # Obtain a single state representation for the whole batch
        batch_state_encoding = self._stack_state_encodings(list_state_encodings)

        return list_state_encodings, batch_state_encoding


    def forward(self, problems:List[Union[PDDLProblem, Any]], applicable_actions_list:List[Tuple[Action]]) \
        -> Tuple[List[torch.Tensor], List]:
        """
        We assume all the problems are represented as either a PDDLProblem or tensor list. We cannot mix both encodings
        in the same problem list.
        """
        if isinstance(problems[0], PDDLProblem):
            list_state_encodings, batch_state_encoding = self.obtain_internal_state_encodings(problems)
        else:
            list_state_encodings = problems
            batch_state_encoding = self._stack_state_encodings(list_state_encodings)
