# File adapted from: https://github.com/fawcettc/planning-features/blob/master/extractors/sas.py
# All credits to originals authors
...
'''
# Note: GNU Affero GPL License is compatible with MIT License
...
class SASFeatureExtractor(FeatureExtractor):
    ...
    def __init__(self):
        super(SASFeatureExtractor, self).__init__()

        self.extractor_name = "sas"

        # self.sas_graph_features = args.sas_graph_features
        self.sas_graph_features = False # Change to True if we can extract Cenamor's features
    ...
    def extract(self, fd_path, domain_path, instance_path):
        features = self.default_features()

        if fd_path[-1] in ('/', '\\'):
            fd_path = fd_path[:-1] # Remove \ and / from the last position of fd_path

        # path_to_translate = "%s/fast-downward/translate/translate.py" % (self.abs_script_directory)
        path_to_translate = "{}/src/translate/translate.py".format(fd_path)

        translate_cmd = ["python", path_to_translate, '--sas-file', fd_path + '/output.sas', domain_path, instance_path] # --sas-file option to change the location where output.sas is created
        ...
        try:
            ...
            translate_output = subprocess.run(translate_cmd, shell=False, stdout=subprocess.PIPE).stdout.decode('utf-8')
            translate_stdout_features = self.extract_stdout_features(translate_output) # Output translate.py
            features.update(translate_stdout_features)

            sas_path = "{}/output.sas".format(fd_path)

            if not os.path.exists(sas_path):
                return Exception("output.sas does not exist")

            sas_file_features = self.extract_sas_file_features(sas_path) # output.sas
            features.update(sas_file_features)

            # extract the preprocessing features
            # Call ./fast-downward/preprocess/preprocess < output.sas
            # Doesn't work on Windows!
            # In the newer versions of FD, some features are on the output of translate.py (the other features are very similar to those of translate.py)
            preprocess_stdout_features = self.extract_preprocess_stdout_features(translate_output) # Output translate.py
            features.update(preprocess_stdout_features)

            # We do not extract the sas_graph_features (we can't on Windows)
    ...
    # Parse output of translate.py
    # Command: python fast-downward\src\translate\translate.py logistics-domain.pddl logistics-problem-hard.pddl
    # All features except sasImpliedEffectsRemoved
    def extract_stdout_features(self, output):
        ...
        sas_implied_effects_removed_match = re.search("([0-9]*) implied effects removed", output) # Does not appear on new versions of FD
    ...
    # File: output.sas (created automatically after calling translate.py)
    # Works perfectly (didn't check mutexes though)
    def extract_sas_file_features(self, sas_file):
        ...
        with open(sas_file, 'r') as f:
            ...
            # -- Parsing of output.sas ends here
    ...
    # Parse output of preprocess
    # Note: we parse the output of translate.py on the new versions of FD
    def extract_preprocess_stdout_features(self, preprocess_output):
        preprocess_stdout_features = {}

        # I can obtain the next four features by calling the new version of FD and parsing the output of translate.py
        deemed_necessary_match = re.search("([0-9]*) of ([0-9]*) variables necessary", preprocess_output)
        ...
        # Equal to Translator derived variables
        ...
        # Similar to Translator task size
    ...
    # Parse file "output"
    # File only created after calling preprocess on old versions of FD!!
