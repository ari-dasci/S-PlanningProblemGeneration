...
// Adapted by Carlos Núñez Molina
...
void output_typed_strips(){
  ...
  /* header
   */
  printf("(define (problem sokoban-instance-generator)");
  printf("\n(:domain sokoban)");
...
void print_goal_predicates(){
 for(int i=0;i<goal_locs.size();++i){
  ...
  printf("\n(at-box f%d-%df) ", goal_locs[i].first,goal_locs[i].second);
 }  
}
...
void print_box_predicates(){
 for(int i=0;i<box_locs.size();++i){
  ...
  printf("\n(at-box f%d-%df)", box_locs[i].first,box_locs[i].second);
 }
}
  ...
  for (int i=0; i < num_rows; ++i) {
    for (int j=0; j < num_cols; ++j) {
     ...
     if(j+1<=num_cols-1){
      ...
      printf("\n(connected-right f%d-%df f%d-%df)",i,j,i,j+1);
     }
     //up connectivity
     if(i-1>=0){
      //printf("\n(adjacent f%d-%df f%d-%df up)",i,j,i-1,j);
      printf("\n(connected-up f%d-%df f%d-%df)",i,j,i-1,j);
     }/*
     //down connectivity
     if(i+1<=num_rows-1){
